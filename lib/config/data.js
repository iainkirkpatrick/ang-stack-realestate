        ...
        request('http://api.tmsandbox.co.nz/v1/Search/Property/Residential.json?page=' + page, function (error, response, body) {
          ...
          if (!error && response.statusCode === 200) {
            ...
            _.forEach(JSON.parse(body).List, function(listing) {
              //deal with listings with/without pics
              var picstr;
              if (typeof(listing.PictureHref) === "undefined") {
                picstr = 'http://www.tmsandbox.co.nz/images/NewSearchCards/GVIcons/noPhoto_Gray.gif';
              } else {
                picstr = listing.PictureHref.replace('thumb', 'tq');
              }
              // var picstr = listing.PictureHref;
              // console.log(typeof(picstr));
              // var arg = picstr.replace('thumb', 'tq');
              // // .replace('thumb', 'tq');
              secondaryProp.create({
                ...
                location: { type: "Point", coordinates: [ listing.GeographicLocation.Longitude, listing.GeographicLocation.Latitude ] }
              });
            });
